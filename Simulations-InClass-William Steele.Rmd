...
gamblers_ruin =function(bet,bankroll,prob) { #creating function gamblers_ruin
  ...
  while(bet>0) { #while the bet is still positive, hence above 0
    ...
    count = count + 1   
    if(bankroll < bet) { #if your bankroll is less than bet
      break
      }   
  }
...
probability = length(which(durations<=100))/total
probability
    ...
    ``` {r}
total_bets = length(durations)
prob_bust_by_500 = length(which(durations<=500))/total
prob_bust_by_500
